...
OPTS = {"tmdl-areas": {"offset": 0.0004, "single": True}, "pwi-basins": {"offset": 0.00006}, "pwi-lines": {"offset": 0.00006}, "imp-lakes": {"offset": 0.00006},
        "fema-zones": {"offset": 0.0001, "oid": "OBJECTID", "ov_offset": 0.0005}, "fema-xs": {"oid": "OBJECTID"}, "fema-bfe": {"oid": "OBJECTID"}, "fema-lomr": {"oid": "OBJECTID"},
...
def write_overview(lid, feats):
    ...
    ov = feats if is_point else page(lid, OPTS.get(lid, {}).get("ov_offset", OV_OFFSET), OPTS.get(lid, {}).get("ov_where"))
