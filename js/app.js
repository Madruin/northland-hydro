...
const state = {
  ...
  layers: { contam: false, trails: false, huc: false, landcover: false, stations: true, gauges: true, qpe: false, streams: false, soils: false, parcels: false, trout: false, karst: false, wetlands: false, fema: false, crossings: false, wells: false, pwi: false, impaired: false, easements: false }, qpeWindow: "24h", selection: null, terrain: {}, terrainOpacity: 0.6,
  ...readUrl(),
};
...
function buildControls() {
  ...
  for (const k of ["contam", "trails", "huc", "landcover", "wetlands", "trout", "karst", "fema", "crossings", "wells", "pwi", "impaired", "easements"]) { $("tg-" + k).classList.toggle("on", !!state.layers[k]); $("tg-" + k).addEventListener("click", () => toggleLayer(k)); }
...
function toggleLayer(name, force) {
  ...
  const btn = { contam: "tg-contam", trails: "tg-trails", huc: "tg-huc", landcover: "tg-landcover", stations: "tg-precip", gauges: "tg-gauges", qpe: "tg-qpe", streams: "tg-streams", soils: "tg-soils", parcels: "tg-parcels", trout: "tg-trout", karst: "tg-karst", wetlands: "tg-wetlands", fema: "tg-fema", crossings: "tg-crossings", wells: "tg-wells", pwi: "tg-pwi", impaired: "tg-impaired", easements: "tg-easements" }[name];
  if (name === "crossings") setCrossingsEnabled(on);
  if (name === "wells") setWellsEnabled(on);
  if (["trout", "karst", "wetlands", "pwi", "impaired", "easements", "trails", "contam"].includes(name)) setDnrLayerEnabled(name, on);
  ...
  const hasOverview = ["pwi", "impaired", "easements", "trout", "karst", "crossings", "fema", "trails", "contam"].includes(name);
...
function renderLegend() {
  ...
  for (const k of ["contam", "trails", "pwi", "impaired", "easements", "wetlands", "trout", "karst"]) if (state.layers[k]) lg.insertAdjacentHTML("beforeend", dnrLegendHtml(k));
...
async function boot() {
  ...
  for (const k of ["pwi", "impaired", "easements", "trails", "contam"]) if (state.layers[k]) { if (map.getSource("ov-rim")) setDnrLayerEnabled(k, true); else on("map:ready", () => setDnrLayerEnabled(k, true)); }
