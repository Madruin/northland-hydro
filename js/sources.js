...
export const SOURCES = [
  { group: "Rainfall and weather", items: [
    ...
    { name: "WLSSD rainfall overview (Western Lake Superior Sanitary District)", fresh: ["wlssd"], url: "http://extranet.wlssd.com/plantoverview/rain.aspx", use: "Nine plant-telemetry rain gauges around Duluth (WLSSD Plant, Scanlon, Endion, Munger Trail, Wrenshall, Pike Lake, Rice Lake, Proctor, Hermantown): this hour, last hour, today and yesterday, snapshotted hourly by a scheduled job into the site because the page allows no direct browser access; daily totals accumulate into a history for window totals.", note: "Days are midnight to midnight, unlike the 7 am observer days of CoCoRaHS and COOP. Gauge locations are approximate." },
    ...
    { name: "NWS MRMS base reflectivity (live radar mosaic)", url: "https://radar.weather.gov/", use: "Live radar option of the Radar layer: the 1 km Multi-Radar Multi-Sensor base reflectivity mosaic behind radar.weather.gov, re-pulled every 3 minutes; dBZ legend from the NWS RIDGE scale." },
    { name: "NWS River Forecast Center QPE", fresh: ["qpe"], url: "https://mapservices.weather.noaa.gov/raster/rest/services/obs/rfc_qpe/MapServer", use: "Radar QPE layer: multi-sensor precipitation estimate for 1 h to 24 h and since 12Z, drawn as NWS map tiles." },
  ...
  { group: "Rivers, lakes and gauges", items: [
    ...
    { name: "MN DNR dams inventory", fresh: ["dams"], url: "https://gis.data.mn.gov/datasets/e21f9058e0504a99b118273c8a433b7d", use: "Outlet structures near a lake: spillway and top-of-dam elevations, hazard class, datum." },
    { name: "MN DNR hydrography (lake and basin polygons)", fresh: ["hydrography"], url: "https://gis.data.mn.gov/search?q=DNR%20hydrography", use: "Lake outline for the clicked point and the Lake section." },
    { name: "MN DNR Public Waters Inventory", fresh: ["pwi", "snapshots"], url: "https://www.dnr.state.mn.us/waters/watermgmt_section/pwi/index.html", use: "Public waters layer and point section: public water basins and wetlands with DNR shoreland class, and public watercourses; a public waters work permit is needed for bed or bank work below the OHWL.", note: "GIS delineations; the official PWI maps and the area hydrologist govern." },
    { name: "MPCA impaired waters (2024 303(d) list, EPA-approved; the draft 2026 list closed comment July 2026 and awaits EPA approval) and TMDL allocation areas", fresh: ["impaired", "tmdl"], url: "https://www.pca.state.mn.us/air-water-land-climate/minnesotas-impaired-waters-list", use: "Impaired layer, point section (nearest reaches and lakes with impairments, affected uses, TMDL approved or needed, allocation areas containing the point) and the watershed section's list of impaired waters in the basin." },
    { name: "BWSR RIM conservation easements and wetland banking easements", fresh: ["rim", "wetbank"], url: "https://bwsr.state.mn.us/easements", use: "Easements layer and point section with easement number, type, acres, year, status and SWCD." },
    { name: "MN DNR Cooperative Groundwater Monitoring (observation wells)", fresh: ["obwells"], url: "https://www.dnr.state.mn.us/waters/cgm/index.html", use: "Observation wells on the Wells layer and in the point section, with aquifer, depth, read status and a link to each well's water-level record." },
    { name: "MN DNR designated trout streams (Minn. Rules 6264.0050, last amended effective March 1, 2026)", fresh: ["trout"], url: "https://gis.data.mn.gov/datasets/43392623c7dc471da0b84615b3a0faa4", use: "Trout layer: designated streams and tributary reaches." },
  ] },
  ...
  { group: "Terrain and imagery", items: [
    ...
    { name: "MnTOPO 2nd-generation lidar (USGS 3DEP, 2021–2024)", fresh: ["lidar"], url: "https://www.mngeo.state.mn.us/chouse/elevation/lidar.html", use: "0.5 m hillshade rendered live; point elevation and line profiles from the seamless DEM image service (identify and getSamples); DEM clips and contours for the CAD export from the cloud-optimized GeoTIFFs (NAVD88). The 1 m 2008–12 DEM is the fallback where the new lidar is missing." },
    ...
    { name: "MnGeo Geospatial Image Service (WMS)", fresh: ["imagery"], url: "https://www.mngeo.state.mn.us/chouse/wms/geo_image_server.html", use: "Imagery basemaps served as Web Mercator tiles from the wmsll endpoint. The default Imagery is the newest statewide flight (USDA FSA 2025, about 60 cm). \"Sharpest\" stacks the highest-resolution orthophotos over it: Lake County 2024 (6 in), Carlton County 2021 (6 in), north-central Minnesota 2013 (1 ft, west of about −92.2°), Duluth and Arrowhead 2009. The \"Imagery by year\" group offers every FSA flight back to 2003, the county orthophoto years, and the 1991 USGS black-and-white quads.", note: "The sharpest photo of a place can be older than the newest; the basemap label always states the year." },
  ...
  { group: "Soils, land and wetlands", items: [
    { name: "USDA NRCS SSURGO via Soil Data Access", fresh: ["ssurgo"], url: "https://sdmdataaccess.sc.egov.usda.gov/", use: "Hydrologic soil group layer, map-unit and component details at a point (drainage, hydric, flooding, water table, texture, K factor, Ksat, restriction), basin-wide HSG breakdown and composite TR-55 curve numbers.", note: "Dual groups (A/D etc.) count as their undrained letter unless drained. Web Soil Survey is the NRCS product these data come from." },
    { name: "County tax parcels", fresh: ["parcels_stlouis", "parcels_cook", "parcels_lake", "parcels_carlton", "parcels_aitkin", "parcels_millelacs", "parcels_kanabec", "parcels_pine"], url: "https://gis.data.mn.gov/search?q=tax%20parcels", use: "Parcel boundaries and owner names from the St. Louis, Cook, Lake, Carlton, Aitkin, Mille Lacs and Kanabec county GIS services; Pine County from a parcel export supplied by the Pine County Auditor's Office (2026-09-09), served from this site.", note: "Parcel data are for reference, not survey. Pine County ownership is as of the export date." },
    { name: "MN National Wetlands Inventory update (MN DNR, 2009–2014 imagery)", fresh: ["nwi"], url: "https://www.dnr.state.mn.us/eco/wetlands/nwi_proj.html", use: "Wetlands layer and point section with Cowardin code, Circular 39 type, plant community and hydrogeomorphic class.", note: "Inventory mapping; regulatory boundaries need a field delineation." },
    ...
    { name: "Minnesota Geological Survey / DNR karst features and springs", fresh: ["karst_pts", "karst_polys", "springs"], url: "https://gis.data.mn.gov/search?q=karst", use: "Karst layer: karst-prone bedrock, sinkholes, stream sinks, karst windows and springs." },
  ] },
  { group: "Flood hazard and infrastructure", items: [
    ...
    { name: "MN DNR Culvert Inventory Suite", fresh: ["culverts"], url: "https://gis.data.mn.gov/datasets/3442c914daa74811a320ce5a8f97142c", use: "Stream-crossing surveys: span vs bankfull, passage, condition, priority, corrective actions; culvert openings and bridge assessments joined by crossing id." },
    { name: "FHWA National Bridge Inventory (NTAD, FHWA file as of June 2025)", fresh: ["nbi"], url: "https://geodata.bts.gov/datasets/national-bridge-inventory", use: "MnDOT, county, township and city bridges and culverts over 20 ft: type, year, dimensions, condition ratings, scour status, owner, load ratings." },
  ] },
  { group: "Subsurface", items: [
    { name: "Minnesota County Well Index (Minnesota Geological Survey and MDH)", fresh: ["cwi"], url: "https://cse.umn.edu/mgs/cwi", use: "Wells layer colored by depth to bedrock; driller logs interpreted by MGS (stratigraphy, lithology), static water levels, construction; basin depth-to-bedrock statistics.", note: "Well locations range from GPS to section-level; the location method is shown on each card. Wells cluster along roads and shorelines, so basin statistics are a biased sample." },
    ...
    { name: "MN DNR Drill Core Library boring locations", fresh: ["drillholes"], url: "https://www.dnr.state.mn.us/lands_minerals/dc_library.html", use: "Mineral exploration, scientific and engineering drill holes with core or logs held in Hibbing." },
  ] },
...
export const METHODS = [
  ...
  ["Design-storm runoff", "Basin composite curve number (SSURGO HSG shares × a chosen cover, AMC II) with the point's Atlas 14 24-hour depths in the NRCS runoff equation (Ia = 0.2S); time of concentration from the SCS lag equation using the StreamStats longest flow path and mean basin slope; peak from the TR-55 graphical method with Type II unit-peak curves and the pond/swamp factor. Screening only; NRCS Minnesota designs use the MSE3 distribution here."],
...
async function applyFreshness(el) {
  let f = null; try { const r = await fetch("data/freshness.json"); if (r.ok) f = await r.json(); } catch { f = null; }
  const head = el.querySelector("#fresh-head"); if (!f) { if (head) head.textContent = "Automated freshness check not available."; return; }
  const n = Object.values(f.sources).filter((s) => s.status === "ok").length, total = Object.keys(f.sources).length;
  if (head) head.innerHTML = `Automated freshness check ${escapeHtml(f.checked.slice(0, 10))}: ${n} of ${total} sources current${f.flags.length ? `; <b class="fresh-flag">${f.flags.length} flagged</b>: ${f.flags.map(escapeHtml).join("; ")}` : ""}. Chips below show each source's newest record or publication date.`;
  for (const sp of el.querySelectorAll("[data-fresh]")) {
    const r = f.sources[sp.dataset.fresh]; if (!r) { sp.remove(); continue; }
    sp.className = `fresh fresh-${r.status}`; sp.title = r.detail; sp.textContent = `${r.label.replace(/ (County )?parcels$/, "")}: ${r.asOf || (r.lastGood ? `last seen ${r.lastGood}` : "check failed")}`;
  }
}
export function renderSources(el) {
  el.innerHTML = `<p>Everything on the map comes from the public services below. The slow-changing regional layers (PWI, impaired waters, easements, trout, karst, springs, DNR culvert surveys) draw first from monthly snapshots kept in the site and are then replaced by the live MnGeo data as soon as it answers; the legend note says which one you are seeing. Everything else is fetched live. Each Point panel section and the printed report name the source and retrieval time of what they show.</p><p class="small" id="fresh-head">Checking source freshness…</p>
    ${SOURCES.map((g) => `<h4>${escapeHtml(g.group)}</h4><ul class="src">${g.items.map((it) => `<li><b><a href="${escapeHtml(it.url)}" target="_blank" rel="noopener">${escapeHtml(it.name)}</a></b> — ${escapeHtml(it.use)}${it.note ? ` <span class="small">${escapeHtml(it.note)}</span>` : ""}${(it.fresh || []).map((id) => `<span class="fresh" data-fresh="${id}"></span>`).join("")}</li>`).join("")}</ul>`).join("")}
