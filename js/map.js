...
const OVERLAYS = { "fema-zones": "fema-fill", "fema-lomr": "fema-lomr", wetlands: "fill", trout: "line", "karst-poly": "fill", "karst-pts": "point", springs: "point", "fema-bfe": "fema-line", "fema-xs": "fema-xs", "xing-dnr": "point", "xing-nbi": "diamond", wells: "point", drillholes: "diamond", wlssd: "point" };
const overlayData = {};
...
async function addOverlays() {
  ...
  for (const [id, kind] of Object.entries(OVERLAYS)) {
    ...
    const vis = (id === "trout" ? visibility.trout : id === "wetlands" ? visibility.wetlands : id.startsWith("fema") ? visibility.fema : id.startsWith("xing") ? visibility.crossings : id === "wells" || id === "drillholes" ? visibility.wells : id === "wlssd" ? visibility.stations : visibility.karst) ? "visible" : "none";
...
function setupHover() {
  ...
  map.on("mousemove", (e) => {
    ...
    if (!feats.length) { const ov = ["ov-wlssd", "ov-wells", "ov-drillholes", "ov-xing-dnr", "ov-xing-nbi", "ov-karst-pts", "ov-springs", "ov-fema-xs", "ov-fema-bfe", "ov-trout", "ov-fema-lomr", "ov-karst-poly", "ov-wetlands", "ov-fema-zones"].filter((l) => map.getLayer(l) && map.getLayoutProperty(l, "visibility") === "visible"); if (ov.length) { feats = map.queryRenderedFeatures(e.point, { layers: ov }); anchor = e.lngLat; } }
