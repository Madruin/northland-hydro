...
export const LAYERS = {
  ...
  contam: {
    label: "Contamination (MPCA)", agency: "MPCA", minZoom: 11, live: false, overviewNote: "all kept sites and every area polygon",
    note: "MPCA What's In My Neighborhood sites (cleanup, tanks and hazardous waste, permitted facilities; construction-stormwater-only and licence-only records left out), Groundwater Contamination Atlas areas of concern, source areas and plume boundaries, institutional control areas (recorded land-use restrictions) and closed-landfill waste footprints. Snapshot kept in the site, rebuilt monthly. Pale dots are inactive (closed) records; a closed leak site can still leave residual contamination.",
    sources: [
      { id: "mpca-landfill", url: "", fields: "", kind: "fill", live: false },
      { id: "mpca-ic", url: "", fields: "", kind: "fill", live: false },
      { id: "mpca-gwconcern", url: "", fields: "", kind: "fill", live: false },
      { id: "mpca-gwline", url: "", fields: "", kind: "line", live: false },
      { id: "mpca-sites", url: "", fields: "", kind: "point", live: false },
    ],
    legend: `<div class="legend-row"><span class="swatch" style="background:#dc2626"></span>Cleanup: leak site, petroleum remediation, brownfield, Superfund, voluntary cleanup</div><div class="legend-row"><span class="swatch" style="background:#ea580c"></span>Tanks and hazardous waste</div><div class="legend-row"><span class="swatch" style="background:#6366f1"></span>Permitted facility (wastewater, industrial, feedlot, air)</div><div class="legend-row"><span class="swatch" style="background:#fca5a5"></span>Pale = inactive / closed</div><div class="legend-row"><span class="swatch sq" style="background:#9333ea;opacity:.5"></span>Groundwater contamination area or plume boundary</div><div class="legend-row"><span class="swatch sq" style="background:#be123c;opacity:.5"></span>Institutional control area (land-use restriction)</div><div class="legend-row"><span class="swatch sq" style="background:#57534e;opacity:.5"></span>Closed landfill waste footprint</div>`,
  },
  trails: {
...
function decorate(id, p) {
  ...
  if (id === "mpca-sites") { const on = p.active !== "N"; p.color = { cleanup: on ? "#dc2626" : "#fca5a5", waste: on ? "#ea580c" : "#fdba74", permit: on ? "#6366f1" : "#c7d2fe" }[p.cat] || "#6b7280"; p.popup = `<div class="popup-title">${escapeHtml(p.name || "MPCA site")}</div><div class="popup-sub">${escapeHtml(p.acts || { cleanup: "Cleanup site", waste: "Tanks / hazardous waste", permit: "Permitted facility" }[p.cat] || "")} · ${on ? "active" : "inactive / closed"}${p.ic ? " · <b>institutional controls</b>" : ""}</div>${p.addr || p.naics ? `<div class="popup-sub">${escapeHtml([p.addr, p.naics].filter(Boolean).join(" · "))}</div>` : ""}`; }
  if (id === "mpca-gwconcern" || id === "mpca-gwline") { p.color = "#9333ea"; p.opacity = 0.22; p.popup = `<div class="popup-title">Groundwater ${escapeHtml(p.kind || "contamination area")}</div><div class="popup-sub">${escapeHtml(p.project_name || p.project_na || "")} · ${escapeHtml(p.project_type || p.project_ty || "")} · ${escapeHtml(p.media_type || "")} · ${escapeHtml(p.status || "")}${p.acreage ? " · " + fmt(p.acreage, 1) + " ac" : ""}</div>`; }
  if (id === "mpca-ic") { p.color = "#be123c"; p.opacity = 0.25; p.popup = `<div class="popup-title">Institutional control area</div><div class="popup-sub">${escapeHtml(p.ai_name || "")} · ${escapeHtml(p.si_type_desc || "")}${p.si_cat_desc ? " (" + escapeHtml(p.si_cat_desc) + ")" : ""} · ${escapeHtml(p.ai_program || "")}</div><div class="popup-sub">Recorded land-use restriction at a cleanup site; check the MPCA file before excavation or dewatering.</div>`; }
  if (id === "mpca-landfill") { p.color = "#57534e"; p.opacity = 0.3; p.popup = `<div class="popup-title">Closed landfill waste footprint</div><div class="popup-sub">${escapeHtml(p.facilityname || "")} · ${escapeHtml(p.status || "")}${p.acres ? " · " + fmt(p.acres, 1) + " ac" : ""}</div>`; }
...
async function loadOverview(id) {
  if (overviewCache[id] === undefined) {
    overviewCache[id] = fetch(`data/layers/${id}/overview.json`).then(async (r) => { if (!r.ok) { const idx = await fetch(`data/layers/${id}/index.json`).then((x) => (x.ok ? x.json() : null)).catch(() => null); if (!idx?.single) return null; r = await fetch(`data/layers/${id}/${idx.single}`); if (!r.ok) return null; } const fc = await r.json(); for (const f of fc.features) decorate(id, f.properties); return fc; }).catch(() => { delete overviewCache[id]; return null; });
  }
