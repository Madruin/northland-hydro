...
const GLOSSARY = [
  ...
  [/^Reg\. WSEL ft$/i, "Regulatory water-surface elevation of the 1% (base) flood at this cross section, from the effective Flood Insurance Study. A no-rise certification shows the proposed condition does not increase it."],
  [/^XS$/i, "FEMA cross-section letter as shown on the FIRM and in the FIS flood profile. Unlettered sections exist in the model but are not on the printed profile."],
  [/^Streambed ft$/i, "Channel invert elevation at the cross section in the effective model."],
  [/^Station ft$/i, "Stream station along the studied reach, feet from the downstream study limit."],
  [/^From ft$|^To ft$/i, "Depth interval below ground surface at the well, feet, from the driller's log."],
  [/^Driller's description$/i, "Material as written on the well contractor's log; wording and detail vary by driller."],
  [/^Unit$/i, "MGS stratigraphic / aquifer code interpreted from the log (Q = Quaternary glacial, P = Precambrian bedrock, R = surface)."],
  [/^Lithology$/i, "Primary (and secondary) lithology assigned by MGS to the interval."],
  [/^Span ft$/i, "Culvert opening span (width), feet, as measured by the DNR survey."],
  [/^Rise ft$/i, "Culvert opening rise (height), feet."],
  [/^Outlet drop ft$/i, "Drop from the culvert outlet invert to the downstream water surface; a perched outlet is a fish-passage barrier."],
  [/^Inlet \/ outlet$/i, "Inlet and outlet configuration (at stream grade, perched, backwatered, etc.)."],
  [/^Opening$/i, "Pipe or cell number and which end was measured."],
