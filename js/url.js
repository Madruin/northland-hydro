...
export function readUrl() {
  ...
  if (h.get("l") != null) { const l = h.get("l").split(",").filter(Boolean); st.layers = { stations: l.includes("s"), gauges: l.includes("g"), qpe: l.includes("q"), streams: l.includes("r"), soils: l.includes("o"), parcels: l.includes("p"), trout: l.includes("t"), karst: l.includes("k") }; }
...
export function writeUrl(st) {
  ...
  h.set("b", st.basemap);
  h.set("l", [st.layers.stations && "s", st.layers.gauges && "g", st.layers.qpe && "q", st.layers.streams && "r", st.layers.soils && "o", st.layers.parcels && "p", st.layers.trout && "t", st.layers.karst && "k"].filter(Boolean).join(","));
