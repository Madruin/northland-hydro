...
export function writeUrl(st) {
  ...
  h.set("b", st.basemap);
  h.set("l", [st.layers.stations && "s", st.layers.gauges && "g", st.layers.qpe && "q", st.layers.streams && "r", st.layers.soils && "o", st.layers.parcels && "p", st.layers.trout && "t", st.layers.karst && "k", st.layers.wetlands && "w", st.layers.fema && "f", st.layers.crossings && "c", st.layers.wells && "b"].filter(Boolean).join(","));
