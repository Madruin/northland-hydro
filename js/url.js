...
export function readUrl() {
  ...
  if (h.get("l") != null) { const l = h.get("l").split(",").filter(Boolean); st.layers = { stations: l.includes("s"), gauges: l.includes("g"), qpe: l.includes("q"), streams: l.includes("r"), soils: l.includes("o"), parcels: l.includes("p"), trout: l.includes("t"), karst: l.includes("k"), wetlands: l.includes("w"), fema: l.includes("f"), crossings: l.includes("c"), wells: l.includes("b"), pwi: l.includes("v"), impaired: l.includes("i"), easements: l.includes("e"), landcover: l.includes("n"), huc: l.includes("h"), trails: l.includes("l"), contam: l.includes("m") }; }
...
export function writeUrl(st) {
  ...
  h.set("b", st.basemap);
  h.set("l", [st.layers.stations && "s", st.layers.gauges && "g", st.layers.qpe && "q", st.layers.streams && "r", st.layers.soils && "o", st.layers.parcels && "p", st.layers.trout && "t", st.layers.karst && "k", st.layers.wetlands && "w", st.layers.fema && "f", st.layers.crossings && "c", st.layers.wells && "b", st.layers.pwi && "v", st.layers.impaired && "i", st.layers.easements && "e", st.layers.landcover && "n", st.layers.huc && "h", st.layers.trails && "l", st.layers.contam && "m"].filter(Boolean).join(","));
