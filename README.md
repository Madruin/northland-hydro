...
The Point panel has a sticky jump bar with one chip per section (Lake, Watershed, Crossing, FEMA, Wetland, Parcel, Soils, Wells, Rainfall, Nearby, Forecast, Soil moisture, Atlas 14). A chip spins while its section loads, turns solid when there is something to read, and dims when the section has nothing at that point. Plotly is loaded on first chart use rather than at page load, and the Region panel renders from the last cached observations immediately while fresh data loads.
