...
**Imagery** (the default aerial option) is the newest statewide flight, USDA FSA 2025 at about 60 cm, served by MnGeo. **Imagery · sharpest** stacks the highest-resolution orthophotos MnGeo has over that base: Lake County 2024 at 6 in, Carlton County 2021 at 6 in, north-central Minnesota 2013 at 1 ft (west of about −92.2°), and Duluth and the Arrowhead 2009; the sharpest photo of a place can therefore be older than the newest. The **Imagery by year** group in the basemap menu lists every FSA flight back to 2003 with the county orthophotos of that year layered on top (2024 Lake, 2021 and 2019 Carlton and Lake, 2015 Carlton, 2013 north-central, 2009 Arrowhead and Duluth), plus the 1991 USGS black-and-white quads, for looking at how a site has changed. All come from MnGeo's `wmsll` WMS endpoint as 512 px Web Mercator tiles; the map zooms to level 19. Every Point section shows an "as of" time when it finishes loading, and the status line shows when the station data were last refreshed.
...
## Imagery compare and design-storm runoff

The **compare** button on the map's left edge (below measure) opens a swipe divider: the left side is the current basemap, the right side any imagery year from the basemap menu, chosen in the small box at the top; drag the handle to sweep between them. The right side follows the map exactly, so channel migration or bluff retreat between flights reads at a glance.

After a watershed is delineated, the Watershed section adds **Design-storm runoff**: the basin's composite curve number (from its NLCD 2021 land cover crossed with its SSURGO soil groups, or one cover type you choose), runoff depth and volume for the 1- to 100-year 24-hour Atlas 14 storms, and peak flows by the NRCS unit-hydrograph method (as in WinTR-20 and HydroCAD) in three columns: **MSE3** with peak rate factor 484 (MnDOT Tech Memo 15-10-B-02's recommendation), **MSE3 MN** with factor 400 (NRCS Minnesota's option for rural, not-steep basins), and the old **Type II** for comparison with older designs. The implementation reproduces MnDOT's Stearns County HydroCAD example (167 / 145 / 151 cfs) within 1%. It treats the basin as one unit without storage routing, and says so when the basin exceeds 20 mi² or has more than 2% lakes.
...
The search box also finds tax parcels in every county by PIN (dashes and dots optional; the digits alone match too) or by owner name (four or more letters), querying the seven county services and the Pine County index in parallel; choosing a parcel turns on the Parcels layer, flies to it and opens its Point panel. Place results exclude Wisconsin, Michigan and Ontario. The box accepts coordinates as well as names: `46.9476, -91.7849`, `46.9476 N 91.7849 W`, UTM 15N metres `592468, 5200058` or `E 592468 N 5200058`, or UTM 15N US survey feet `1943790, 17060522` (the TSA3 CAD system). Choosing the result drops the point and opens its panel. Every Point panel header shows the location in decimal degrees, UTM 15N metres and UTM 15N US feet with copy buttons; the feet value copies as `E,N` so it pastes straight into AutoCAD as X,Y. NAD83 and WGS84 are treated as the same datum (about a metre apart here).

Under the coordinates, an **Open here in** row links the same spot in SoilWeb (UC Davis SSURGO viewer), the NWS point forecast, the FEMA Map Service Center, Google Maps and driving directions. The last ten points opened are kept in the browser and offered as **Recent point** entries when the search box is focused while empty. The help dialog opens with a three-step quick start and a *Try it on the Knife River* button that turns on Wells, FEMA and Crossings and opens a point at the CR 102 bridge.
...
## Public waters, impaired waters, easements

Three regulatory layers under **Layers ▾**, each with a Point panel section:

- **Public waters (PWI)** (zoom 11+): DNR Public Waters Inventory basins (blue) and wetlands (green) with the DNR shoreland class, and public watercourses (dark blue). The point section names the basin or the nearest watercourse within 150 m and reminds you that bed-and-bank work below the OHWL needs a DNR public waters work permit and that shoreland zoning applies within 1,000 ft of a basin and 300 ft of a watercourse. Lake Superior's basin polygon is excluded from the map layer for speed.
- **Impaired waters & TMDLs** (zoom 10+): MPCA's 2024 303(d) list, stream reaches in red and lakes in orange, hover for the impairment parameters, plus approved TMDL allocation areas in purple. The point section lists the impaired reaches within 500 m and lakes within 300 m with impairments spelled out (mercury in fish tissue, turbidity, E. coli and so on), affected uses, and whether a TMDL is approved or still needed, plus any TMDL allocation area the point falls in. The Watershed section adds **Impaired waters in basin**: every listed reach and lake touching the delineated basin, with a tally of impairments and the TMDL areas overlapping it.
- **Conservation easements** (zoom 10+): BWSR RIM easements (green) and wetland banking easements (teal) with easement number, type, acres, year, status and SWCD.
...
The slow-changing regional layers (trout, karst, springs, easements, impaired waters and TMDL areas, PWI, DNR culvert surveys) are served from snapshots in `data/layers/` built by `tools/build_static_layers.py`, so they draw in under a second regardless of MnGeo; the legend note shows the snapshot date while MnGeo is queried in the background, and as soon as MnGeo answers the live features replace the snapshot and the note changes to "live from MnGeo, h:mm" (if MnGeo times out the snapshot stays, the note says so, and the live check pauses for five minutes). A GitHub Action (`.github/workflows/snapshots.yml`) rebuilds them on the first of every month and commits only if something changed; it can also be run on demand from the repo's Actions tab, and the build refuses to commit if the result is unexpectedly large or incomplete. Wetlands and wells are too large to snapshot and stay live. Below each snapshot layer's normal zoom, a region-wide simplified **overview** (`overview.json`, ~200 m geometry) is drawn instead of nothing, so PWI, impaired waters, easements, trout, karst and DNR crossings can be seen across the whole area at once; the legend and Layers menu say "overview" and full detail returns past the layer's zoom. FEMA hazard zones (SFHA and 0.2%), cross sections, BFEs and LOMRs are snapshotted too (`data/layers/fema-*`, 10 m simplification), so the FEMA layer is snapshot-first with a region-wide zone overview; unshaded X areas only come from FEMA live at zoom 12+. Wetlands zoomed out (below 11) come from the USFWS national wetlands raster (100 m cells; the same 2009 NE Minnesota mapping as the DNR polygons shown from zoom 11). An earlier 20-acre simplified polygon overview distorted shapes and was removed. **Watersheds (HUC)** draws USGS WBD HUC-8, HUC-10 (from zoom 6.5) and HUC-12 (from zoom 9) boundaries and names from site snapshots at every zoom, and the Point panel names the HUCs a point falls in. **Land cover** draws NLCD 2021 from MRLC; in the watershed section the runoff tool's default cover is the basin's actual NLCD mix (MRLC WCS GeoTIFF counted inside the basin, crossed with the SSURGO soil-group split), with each class's TR-55 analog shown and an option to treat wetlands as saturated (CN 98). If MRLC is down the tool falls back to a single cover type. Hovering the Land cover layer shows the class, its TR-55 cover and CNs (MRLC GetFeatureInfo), and every Point panel names the land cover at the point. Wells, parcels and soils have no zoomed-out view. **Contamination (MPCA)** shows MPCA What's In My Neighborhood sites (13,349 in the region after leaving out construction-stormwater-only and business-licence-only records), colored cleanup / tanks and hazardous waste / permitted facility and paler when inactive, plus Groundwater Contamination Atlas areas of concern, source areas and plume boundaries, institutional control areas (recorded land-use restrictions) and closed-landfill waste footprints. The Point panel's Contamination section says whether the point is inside any of those areas, how far the nearest plume boundary is, and lists MPCA sites within 500 ft with links to their MPCA records. Built monthly by `tools/build_mpca.py`. Between zoom 9.5 and each regional layer's live-detail zoom, the full-detail snapshot is drawn for the view (the ~200 m overview is only used further out); FEMA hazard zones use a ~40 m overview below zoom 10, the full-detail snapshot from 10, and live FEMA with cross sections and BFEs from 12. Trails include OpenStreetMap paths tagged private or informal (gray = private or no public access) because they are real trails, often on private land. **Trails** merges MN DNR state trails, state park trails and grant-in-aid snowmobile trails, Superior National Forest trails, and OpenStreetMap paths, bike paths, bridleways and nordic ski trails (31,199 segments; OpenStreetMap supplies the local, club, mountain-bike and ski trails and the Superior Hiking Trail) into one layer colored hiking / multi-use / bike / ski / snowmobile-ATV, with each segment's allowed uses, source and managing agency. Named trails and routes show zoomed out; everything from zoom 11. The Point panel lists trails within 100 ft. Built by `tools/build_trails.py` in the monthly snapshot Action (a failed OpenStreetMap download keeps last month's trails). OpenStreetMap data © OpenStreetMap contributors, ODbL. **Crossings** now draw both inventories region-wide at every zoom (2,061 DNR culvert surveys and 2,693 NBI bridges and large culverts, copies kept in the site with the same fields as the live services, so a crossing's popup is identical zoomed in or out); from zoom 10 the DNR surveys in view are also re-read live from MnGeo and merged in. Every Point panel has a **Federally listed species** section from the USFWS IPaC location API (150 m square around the point): listed, proposed and candidate species, with a flag when the northern long-eared bat or tricolored bat range includes the spot, critical habitat in the footprint, Federal Register links and the lead FWS field office; it prints in the site report. Map clicks, search and layers work while station rainfall is still loading; only the nearest-observers table waits for it. The snapshot box covers all eight TSA3 counties (−93.85 to −89.45, 45.50 to 48.65). The Terrain menu has a **Contours, USGS (interval by scale)** option that draws 3DEP contours at any zoom (small-scale, 100 ft, 50 ft, then full) for whole-area views; the MnGeo lidar contours remain the site-work layers from zoom 12. Each source's vintage is listed in the Sources & methods tab; and `tools/check_freshness.py` re-checks every source on the 2nd of each month (`.github/workflows/freshness.yml`), writes `data/freshness.json`, shows an "as of" chip per source in that tab, and fails the workflow (which emails the repo owner) when a source goes stale or a newer edition appears. The Point panel's PWI, impaired-waters and easement sections read the same snapshots first and are then refreshed from MnGeo. The Radar menu has two kinds of product: **Live radar** (the NWS MRMS base reflectivity mosaic behind radar.weather.gov, in dBZ, re-pulled every 3 minutes) and **Rain totals** for a window (NWS RFC multi-sensor QPE in inches, hourly, 1–2 h behind). The legend names which one is on screen, its color classes, and the mosaic's valid time. These MnGeo services can be slow at times (tens of seconds). While a layer loads, the bottom-left status pill lists it ("Loading Public waters, Impaired…") and its row in the Layers menu shows a spinner; a load that fails or times out after 90 s turns the row red with "failed · toggle to retry" and puts the reason in the legend note.
...
## Field notes (points, tracks, photos on the device)

The **Field** tab (also the pin button in the map's left tool column) is built for a phone on site. Everything is stored in the browser's IndexedDB (`nh-field`: `items` for points, tracks and photo metadata; `blobs` for the photo JPEGs), nothing is uploaded, and `navigator.storage.persist()` is requested on the first save. Syncing to team projects is the planned second phase.

- **Point at my location** watches GPS for up to 8 s (or until three fixes and one at ±4 m or better), drops fixes worse than twice the best, and averages the rest weighted by 1/accuracy². The stated accuracy is the best single fix, because phone GPS errors are correlated and averaging does not shrink them. **Tap to add point** uses the map's pick mode.
- **Tracks**: `watchPosition` with high accuracy; fixes worse than ±50 m are skipped, and a new vertex needs to be at least 3 m, or half the accuracy, from the last. A gap over 60 s (screen locked, app switched; browsers stop location in the background) starts a new segment. The Wake Lock API keeps the screen on and is re-acquired when the page becomes visible. Each fix is written to IndexedDB as it arrives, so a crash loses nothing; an interrupted track can be continued with ▶. A track that ends within 30 m of its start reports the enclosed area (UTM shoelace). A banner over the map shows recording status and pick-mode instructions while the sheet is collapsed.
- **Photos** from the gallery (`<input type=file multiple>`): exifr 7.1.3 (loaded on first use) reads GPS latitude/longitude, altitude, `GPSImgDirection` and `DateTimeOriginal`. Don't pass exifr's `pick` option: it stops exifr deriving `latitude`/`longitude`. Each photo is stored as a 1600 px JPEG plus a 360 px thumbnail. Photos without GPS (iOS's photo picker can strip location; the Files picker keeps it) are listed with a **Place** button.
- **Exports**: KMZ (doc.kml with Points / Tracks / Photos folders, photos embedded under `files/` and shown in the balloons); CAD ZIP (`field_points_PNEZD.csv` for Civil 3D's "PNEZD (comma delimited)" import, `field.dxf` with FIELD_POINTS, FIELD_PHOTOS and FIELD_TRACKS layers, README) in NAD83 UTM 15N US survey feet; CSV of every point, photo and track vertex with UTM feet and lat/lon; GPX 1.1. Point and photo elevations are lidar ground (MnTOPO, NAVD88 ft) fetched at export time and cached on the item; 0 in the PNEZD file where no lidar exists. Track vertices are 2D in CAD.

## Site hydrology report

**Print site report** (Point panel) or **Print report** (project detail) opens `report.html`: a letter-size, print-ready summary with a map snapshot, the current window's rainfall at the nearest stations and PRISM point totals with percent of normal and Atlas 14 return periods, nearest gauges with flow class, NWS forecast and QPF, active alerts, the Atlas 14 design-storm table, the watershed analysis (live, or the project's latest saved one) with basin characteristics and regression flows, the regional-curve bankfull dimensions with the other curves for comparison, and a numbered sources-and-methods list with retrieval times and a permalink that reproduces the view. "Download JSON" saves the same document as data. Use the browser's print dialog to save a PDF; enable background graphics. Section 7, *Site conditions at the point*, reproduces the Lake, Crossing, FEMA, Wetland, Parcel, Soils and Wells sections exactly as they were rendered in the Point panel when the report was generated (charts and buttons stripped, collapsed well cards opened), so the printed briefing carries everything the panel showed.
