...
After a watershed is delineated, the Watershed section adds **Design-storm runoff (TR-55 screening)**: the basin's composite curve number from its SSURGO hydrologic-soil-group shares and a cover you choose (woods, pasture, row crop, open space, and so on; a checkbox treats dual-group soils as drained), the 1- through 100-year 24-hour Atlas 14 depths at the point, NRCS runoff depth and volume, a time of concentration from the SCS lag equation (StreamStats longest flow path and mean slope), the pond-and-swamp factor, and a peak from the TR-55 graphical method. The peaks use the Type II unit-peak curves; NRCS Minnesota designs with the MSE3 distribution in this region, so the numbers are for screening and comparison with the gauge-based regression peaks above them, not for design.
