...
10. **Custom domain**: buy a domain (northlandhydro.org/.com returned no DNS answer on 2026-09-11, so likely unregistered; confirm at the registrar), add a `CNAME` file with the bare domain to the repo root, set registrar DNS (`A` apex → 185.199.108.153 / .109.153 / .110.153 / .111.153; `CNAME www → madruin.github.io`), then GitHub Pages settings → custom domain + enforce HTTPS. Afterwards update Supabase Auth Site URL and redirect URLs, `og:url`/`og:image` in index.html, README links. MnGeo reflects any Origin and NTAD/StreamStats/SDA send `*`, so no data source breaks.
