...
- **Header / layers UX** (2026-09-11): Stations, Gauges and Radar stay as quick toggles; every other overlay lives in a grouped **Layers ▾** menu (Water / Land / Hazards & subsurface) with one-line descriptions, minimum-zoom badges that turn amber with "zoom in" when the map is below that zoom, and an active count on the button. Turning a layer on below its zoom posts a status message. Terrain keeps its own menu. First visit: help modal, then a blue "click anywhere on the map" pill until the first click. Share metadata: Open Graph card `assets/og.png` (1200×630, generated by `tools/make_assets.ps1` with System.Drawing), SVG favicon, apple-touch icon, `manifest.webmanifest` (installable on phones). OG URL is hard-coded to the github.io address; update it when a custom domain lands. Follow-up the same day: zoom-hint status is tracked (`zoomHint`/`normalStatus` in app.js) and reverts on moveend once the zoom is reached or the layer is turned off; hint pill dismisses on any pointerdown on the map container (capture phase) as well as map click; Lake Superior chip removed from the header (level lives in the Region panel; lake.js guards the missing elements); subtitle reworded; right panel has a drag handle on its left edge (`#panel-resize`, 300 px to 70% of the window, saved as `nh-panel-w`), the phone bottom sheet can be dragged to any height by its header (custom height, tab taps suppressed after a drag); A−/A+ text-size buttons were tried and removed the same day (unclear, and they pushed the collapse chevron off screen). FEMA point section no longer throws where a FIRM panel exists without a zone polygon (open water). The hint pill's `display:flex` rule beat the `hidden` attribute (same trap as the help modal on day 1): every floating element that toggles `hidden` needs an explicit `[hidden] { display: none !important }` rule.
...
9. UX rounds driven by team feedback; citations on every number. Done 2026-09-11: grouped Layers menu, first-visit hint, share card/icons/manifest.
10. **Custom domain**: `northland.eco` bought by Matías at Porkbun 2026-09-11 (.eco needs the go.eco profile activation before it resolves). `CNAME` file with the bare domain is in the repo root; og:url/og:image and export README point at it. Remaining on Matías: Porkbun DNS, Pages custom-domain setting + HTTPS, Supabase URLs. Original plan: add a `CNAME` file with the bare domain to the repo root, set registrar DNS (`A` apex → 185.199.108.153 / .109.153 / .110.153 / .111.153; `CNAME www → madruin.github.io`), then GitHub Pages settings → custom domain + enforce HTTPS. Afterwards update Supabase Auth Site URL and redirect URLs, `og:url`/`og:image` in index.html, README links. MnGeo reflects any Origin and NTAD/StreamStats/SDA send `*`, so no data source breaks.
...
### Subsurface / boring data (explored and built 2026-09-11)
...
- **Built** (`js/wells.js`, layer letter `b`): Wells layer (zoom 12+) from CWI Allwells colored by depth to bedrock + DNR drill holes as diamonds; point section with 5 nearest wells, decoded C5ST logs (AQUIFER table for unit codes, LTH CODE for lithology, USE/LOC MC/ELEV MC decoded), latest C5WL static level, MDH record link; watershed section with basin depth-to-bedrock stats (polygon query on Allwells, paged to 8,000). Quaternary strat codes such as QCUR/QPUB are not in the AQUIFER table and are shown raw with the era decoded; the full MGS Quaternary code scheme is in the CWI data dictionary (`mgsweb2.mngs.umn.edu/cwi_doc/cwidoc.htm`) if a decoder is wanted. MGS county-atlas boring layers not loaded (they carry CWI relateids, so they largely duplicate CWI points).
